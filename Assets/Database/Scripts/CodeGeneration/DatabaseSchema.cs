    ...
    public class DatabaseSchema
    {
        ...
        private void LoadResources(string path)
        {
            ...
            foreach (var resource in assets)
            {
                try
                {
                    Debug.Log("Processing file " + resource.name);
                    ...
                    if (string.IsNullOrEmpty(typeInfo.name))
                        throw new InvalidSchemaException("Object name cannot be empty - " + resource.name);
                    if (string.IsNullOrEmpty(typeInfo.type))
                        throw new InvalidSchemaException("Object type cannot be empty - " + resource.name);
                    ...
                    using (var reader = new System.IO.StringReader(resource.text))
                    {
                        ...
                        {
                            throw new InvalidSchemaException("Unknown item type: " + typeInfo.type + " in " + resource.name);
                        }
                    }
                }
                catch (Exception e)
                {
                    Debug.LogError("Unable to load asset " + resource.name + ": " + e.Message);
                    throw;
                }
            }
        }
        ...
        {
            if (!_enums.TryGetValue(ItemTypeEnum, out var itemTypeEnum))
                throw new InvalidSchemaException(ItemTypeEnum + " enum must be defined");
